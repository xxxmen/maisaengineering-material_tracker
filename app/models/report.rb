...
class Report
  ...
  # CONSTANTS
	...
	#ERROR_OUTPUT = "2>> #{Rails.root}/log/stderr.txt"
...
class PipingClassReport < Report
  ...
  # CLASS METHODS
    def self.all_classes_pdf
		...
		report.report_name = "#{Rails.root}/reports/piping_class_details_no_notes_linux.rep"
	...
	def self.details_pdf(params)
		...
		report = self.new
		report.options = "-paramPCLASSIDS='#{class_id}'"
		report.report_name = "#{Rails.root}/reports/piping_class_details_piping_notes.rep"
		...
		new_pdf = "#{Rails.root}/tmp/tmp_report.pdf"
	...
	def self.valves_pdf(params)
		...
		report.report_name = "#{Rails.root}/reports/valve_sheets_for_piping_class_linux.rep"
  	...
  	def self.notes_pdf
		report = self.new
		report.report_name = "#{Rails.root}/reports/piping_notes_linux.rep"
...
class BillsReport < Report
  ...
  # CLASS METHODS
	...
	def self.request_for_quote_pdf(params)
		...
		case RUBY_PLATFORM
			when /mswin32/
				report.report_name = "\"#{Rails.root}\\reports\\popv_bom_rfq_vert_windows.rep\""
				report.output = nil
			when /cygwin/
				report.report_name = "\"#{Rails.root}\\reports\\popv_bom_rfq_vert_windows.rep\""
			else # *nix
				report.report_name = "#{Rails.root}/reports/popv_bom_rfq_vert_linux.rep"
		end
  	...
  	def self.pdf(params)
		...
		case RUBY_PLATFORM
			when /mswin32/
				report.report_name = "\"#{Rails.root}\\reports\\popv_bom_vert_windows.rep\""
				report.output = nil
			when /cygwin/
				report.report_name = "\"#{Rails.root}\\reports\\popv_bom_vert_windows.rep\""
			else
				report.report_name = "#{Rails.root}/reports/popv_bom_vert_linux.rep"
		end
...
class ValvesReport < Report
  ...
  # CLASS METHODS
	...
	def self.all_pdf(params)
		...
		report.report_name = "#{Rails.root}/reports/valve_sheet_linux.rep"
		report.title = "ALL_VALVES_SHEET.pdf"

		report
#		report_name = "#{Rails.root}/reports/valve_sheet_linux.rep"
